...
FILE *openProcess(const QString &command)
{
    ...
    return _popen(processedCommand.toLocal8Bit().constData(), "r");
}
...
bool scanImports(Options *options, QSet<QString> *usedDependencies)
{
    ...
    FILE *qmlImportScannerCommand = _popen(qmlImportScanner.toLocal8Bit().constData(), "r");
...
bool createAndroidProject(const Options &options)
{
    ...
    for (const QString &libraryProject : libraryProjects) {
        ...
        FILE *androidToolCommand = _popen(androidTool.toLocal8Bit().constData(), "r");
        if (androidToolCommand == 0) {
            fprintf(stderr, "Cannot run command '%s'\n", qPrintable(androidTool));
            return false;
        }
...
bool buildAntProject(const Options &options)
{
    ...
    int errorCode = _pclose(antCommand);
    if (errorCode != 0) {
...
bool buildGradleProject(const Options &options)
{
    ...
    int errorCode = _pclose(gradleCommand);
    if (errorCode != 0) {
        fprintf(stderr, "Building the android package failed!\n");
        if (!options.verbose)
            fprintf(stderr, "  -- For more information, run this command with --verbose.\n");
...
bool uninstallApk(const Options &options)
{
    ...
    int returnCode = _pclose(adbCommand);
    if (returnCode != 0) {
...
bool jarSignerSignPackage(const Options &options)
{
    ...
    int errorCode = _pclose(jarSignerCommand);
    if (errorCode != 0) {
    ...
    errorCode = _pclose(zipAlignCommand);
    if (errorCode != 0) {
...
bool signPackage(const Options &options)
{
    ...
    int errorCode = _pclose(zipAlignCommand);
    if (errorCode != 0) {
        fprintf(stderr, "zipalign command failed.\n");
        if (!options.verbose)
            fprintf(stderr, "  -- Run with --verbose for more information.\n");
    ...
    auto apkSignerRunner = [&] {
        ...
        errorCode = _pclose(apkSignerCommand);
        if (errorCode != 0) {
...
bool deployAllToLocalTmp(const Options &options)
{
    ...
    int errorCode = _pclose(adbCommand);
    if (errorCode != 0) {
