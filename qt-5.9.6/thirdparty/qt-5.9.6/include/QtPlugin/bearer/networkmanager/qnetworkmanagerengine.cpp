    ...
    nmAvailable(false)
{
    ...
    QDBusConnectionInterface *pInterface = QDBusConnection::systemBus().Interface();

    if (!pInterface) return;

    if (pInterface->isServiceRegistered("org.ofono"))
        QMetaObject::invokeMethod(this, "ofonoRegistered", Qt::QueuedConnection);

    if (pInterface->isServiceRegistered(NM_DBUS_SERVICE))
