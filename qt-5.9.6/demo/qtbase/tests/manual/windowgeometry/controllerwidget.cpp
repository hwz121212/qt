...
WindowControl::WindowControl(QWindow *w )
    : BaseWindowControl(w)
    , m_stateControl(new WindowStateControl())
{
    setTitle(w->title());
...
void WindowControl::refresh()
{
    const QWidget *w = static_cast<const QWidget *>(m_object);
    BaseWindowControl::refresh();
    m_stateControl->setVisible(w->isVisible());
...
{
    QWidget *w = static_cast<QWidget *>(m_object);
    w->setVisible(m_stateControl->isVisible());
    w->setWindowState(m_stateControl->windowState());
}
